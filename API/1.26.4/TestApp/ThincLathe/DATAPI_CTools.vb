...
Public Class DATAPI_CTools
    ...
    Private m_blnSupportedExtendedNextReadyStations As Boolean

    Dim m_intExtendedNextReadyStations(5) As Integer
    Dim m_intLargeDiameterStartingPots1 As Integer
    Dim m_intLargeDiameterStartingPots2 As Integer
    Dim m_intLargeDiameterEndingingPots1 As Integer
    Dim m_intLargeDiameterEndingingPots2 As Integer

    Public Sub New()
        m_blnSupportedExtendedNextReadyStations = IsPLC3SpecCodeTurnOn(74, 1)

        m_intExtendedNextReadyStations(0) = 298
        m_intExtendedNextReadyStations(1) = 299
        m_intExtendedNextReadyStations(2) = 300
        m_intExtendedNextReadyStations(3) = 301
        m_intExtendedNextReadyStations(4) = 302

        m_intLargeDiameterStartingPots1 = 245
        m_intLargeDiameterEndingingPots1 = 267
        m_intLargeDiameterStartingPots2 = 291
        m_intLargeDiameterEndingingPots2 = 313
    End Sub
...
#Region "CTools.ToolData Example"
    ...
    Public Enum MethodTypeEnum
        ...
        ToolNumberInGroup3_P300S            'P300S or newer
...
#Region "CTools.ToolData per sub system Example"
    ...
    ''' <summary>
    ''' register new tool data
    ''' </summary>
    ''' <remarks>Sample codes only. It does not detect all specifications of each machine</remarks>
    Public Sub RegisterToolData()
        Dim enSubSystems() As Okuma.CLDATAPI.Enumerations.SubSystemEnum = System.Enum.GetValues(GetType(Okuma.CLDATAPI.Enumerations.SubSystemEnum))
        Dim enSubSystem As Okuma.CLDATAPI.Enumerations.SubSystemEnum
        ...
        For Each enSubSystem In enSubSystems
            If IsValidSubSystem(enSubSystem) Then
                ...
                If enSubSystem = Okuma.CLDATAPI.Enumerations.SubSystemEnum.NC_AL Or enSubSystem = Okuma.CLDATAPI.Enumerations.SubSystemEnum.NC_BL Or enSubSystem = Okuma.CLDATAPI.Enumerations.SubSystemEnum.NC_CL Then
                    If IsP300S_TD() Then
                        'register tools
                        RegisterTools_P300S(GeneratedTestValues(1, GetToolNos(enSubSystem), 100))
                    ElseIf IsP300L() Then
                        'call tool data functions for P300L controls
...
#Region "CTools.ToolData_P300S per subsystem Example"
    ...
    Private Sub ToolData_P300S(ByVal enSubSystem As Okuma.CLDATAPI.Enumerations.SubSystemEnum)
        ...
        'register tools
        RegisterTools_P300S(GeneratedTestValues(1, GetToolNos(enSubSystem), 50))
...
#Region "CTools.Get-Set-ToolWearOffset_P300S Example"
    ...
    ''' </summary>
    ''' <remarks>
    ''' <para>For P300S and newer controls with IsLTMIMSpecOn = True and TD Mode = True,
...
#Region "CTools.Get-Set-ToolLifeStatus Example"
    ...
    ''' </summary>
    ''' <remarks>
    ''' <para>For P300S and newer controls with IsLTMIMSpecOn = True and TD Mode = True, each registered tool number (Not a turret station number) can support tool life status per edge if it is applicable. </para>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = OFF) and IsLTMIMSpecOn = True, each tool number (turret station number) can have its own tool life status.</para>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = ON) and and IsLTMIMSpecOn = True, each edge of each tool number (turret station number) can have its own tool life status.</para>
    ''' <para>For P300L/P300S and newer controls with IsLTMIMSpecOn = false, it will be like P100II/P200</para>
...
#Region "CTools.Get-Set-ToolGaugeStatus Example"
    ...
    ''' </summary>
    ''' <para>For P300S and newer controls with IsLTMIMSpecOn = True and TD Mode = True, each registered tool number (Not a turret station number) can support tool gauge per edge if it is applicable. </para>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = OFF) and IsLTMIMSpecOn = True, each tool number (turret station number) can have its own gauge status</para>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = ON) and and IsLTMIMSpecOn = True, each edge of each tool number (turret station number) can have its own gauge status</para>
    ''' <para>For P300L/P300S and newer controls with IsLTMIMSpecOn = false, it will be like P100II/P200</para>
...
#Region "CTools.ToolLife Example"
    ...
    ''' </summary>
    ''' <para>For P300S and newer controls with IsLTMIMSpecOn = True and TD Mode = True, each registered tool number (Not a turret station number) can support tool life per edge if it is applicable. </para>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = OFF) and IsLTMIMSpecOn = True, each tool number (turret station number) can have its own tool life value.</para>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = ON) and and IsLTMIMSpecOn = True, each edge of each tool number (turret station number) can support tool life per edge.</para>
    ''' <para>For P300L/P300S and newer controls with IsLTMIMSpecOn = false, it will be like P100II/P200</para>
...
#Region "CTools.ToolGroup Example"
    ...
    Private Sub ToolGroup(ByVal enSubSystem As Okuma.CLDATAPI.Enumerations.SubSystemEnum, _
        ...
        If (objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P300SLP Or objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500SLP) And objSpec.IsLTMIMSpecOn() And blnTDMode Then
...
#Region "CTools.ReferenceToolOffset Example"
    ...
    ''' </summary>
    ''' <remarks>
    ''' <para>For P300S and newer controls with IsLTMIMSpecOn = True and TD Mode = True, each tool number (Not a turret station number) can support 3 reference offset numbers named as OG1/2/3</para>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = OFF) and IsLTMIMSpecOn = True, each tool number (turret station number) can only support 2 reference offset numbers named as OFFSET NO.1 AND OFFSET NO.2</para>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = ON) and and IsLTMIMSpecOn = True, each edge of each tool number (turret station number) can support 3 reference offset numbers named as OG1/2/3</para>
    ''' <para>For P300L/P300S and newer controls with IsLTMIMSpecOn = false, it will be like P100II/P200</para>
    ...
    Private Sub ReferenceToolOffset_123(ByVal enSubSystem As Okuma.CLDATAPI.Enumerations.SubSystemEnum, _
        ...
        If objTools.IsLTMIMSpecOn Then
            If (objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P300SLP Or objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500SLP) Then
                If blnTDMode Then
            ...
            ElseIf (objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P300L Or objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500L) Then
...
#Region "CTools.ReferenceToolOffset_12 Example"
    ...
    ''' </summary>
    ''' <remarks>
    ''' <para>For P300L and newer controls with Multi-Tool spec (NC Spec Code 2 [24, 7] MLTL = OFF) and IsLTMIMSpecOn = True, 
...
#Region "CTools.ToolPositionComment Example"
    ...
    ''' </summary>
    ''' <para>For P300S and newer controls with IsLTMIMSpecOn = True</para>
    Private Sub ToolPositionComment(ByVal enSubSystem As Okuma.CLDATAPI.Enumerations.SubSystemEnum, _
        ...
        If (objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P300SLP Or objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500SLP) And objSpec.IsLTMIMSpecOn() Then
...
#Region "CTools.GetSpindleAxisMode Example"
    ...
    Private Function SpindleAxisMode(ByVal enSubSystem As Okuma.CLDATAPI.Enumerations.SubSystemEnum, _
        ...
        ElseIf (objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500SLP Or objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500L) And objSpec.IsLTMIMSpecOn() Then
            enValue = objTools.GetSpindleAxisMode(intToolNo, enEdgeNo, enToolCuttingPosition)
        End If
...
#Region "Helper Functions"
    ...
    Private Function CheckMethodSpec(ByVal enMethodType As MethodTypeEnum, _
        ...
        Select Case enMethodType
            ...
            Case MethodTypeEnum.ToolNumberInGroup3_P300S
                Select Case enSubSystem
                ...
                If GetSpecCode(21, 3) = True _
                    And GetSpecCode(2, 6) = True _
                    And GetSpecCode(3, 4) = True _
                    And GetSpecCode(21, 6) = True _
                    And GetSpecCode(5, 3) = True _
                    And GetSpecCode(6, 6) = True Then
    ...
    ''' P200
    ''' P300L/P500L AND IsLTMIMSpecOn() = true
    ''' P300S/P500S AND IsLTMIMSpecOn() = true AND in TL mode
    ''' </summary>
    ...
    Protected Function IsP200P300LP300S_TL() As Boolean
        ...
        If GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P200 _
            ...
            Or GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500L _
            Or IsP300S_TL() Then
            blnValue = True
        End If
    ...
    Protected Function IsP200() As Boolean
        ...
        If objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P200 _
            ...
            Or (objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500L And objSpec.IsLTMIMSpecOn() = False) _
            Or (objSpec.GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500SLP And objSpec.IsLTMIMSpecOn() = False) _
    ...
    ''' P300L AND IsLTMIMSpecOn() = FALSE ==> will be functions like P100II/P200
    ''' P300S AND IsLTMIMSpecOn() = FALSE ==> will be functions like P100II/P200
    ''' P500L AND IsLTMIMSpecOn() = FALSE ==> will be functions like P100II/P200
    ''' P500S AND IsLTMIMSpecOn() = FALSE ==> will be functions like P100II/P200  
    ''' </summary>
    ''' <returns>True if it is one of the conditions is met</returns>
    ''' <remarks>Sample codes only. It does not detect all specifications of each machine</remarks>
    ...
    ''' <summary>
    ''' Check if the control type and its TD/TL mode can be used for functions designed to work for the following control types and TD/TL mode if applicable:
    ''' P300L/P500L AND IsLTMIMSpecOn() = ON 
    ''' </summary>
    ''' <returns>True if it is one of the conditions is met</returns>
    ''' <remarks>Sample codes only. It does not detect all specifications of each machine</remarks>
    Protected Function IsP300L() As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Dim blnValue As Boolean = False
        If (GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P300L Or GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500L) And objSpec.IsLTMIMSpecOn() = True Then blnValue = True
        Return blnValue
    End Function
    ...
    ''' P300S/P500S AND IsLTMIMSpecOn() = ON and TD Mode = true
    ''' </summary>
    ''' <returns>True if it is one of the conditions is met</returns>
    ''' <remarks>Sample codes only. It does not detect all specifications of each machine</remarks>
    Protected Function IsP300S_TD() As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Dim blnValue As Boolean = False
        If (GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P300SLP Or GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500SLP) And objSpec.IsLTMIMSpecOn() = True And IsTDMode() = True Then blnValue = True
        Return blnValue
    End Function

    ''' <summary>
    ''' Detect if the control type and its TD/TL mode can be used for functions designed to work for the following control types and TD/TL mode:
    ''' P300S/P500S AND IsLTMIMSpecOn() = ON and TD Mode = FALSE
    ''' </summary>
    ''' <returns>True if it is one of the conditions is met</returns>
    ''' <remarks>Sample codes only. It does not detect all specifications of each machine</remarks>
    Protected Function IsP300S_TL() As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Dim blnValue As Boolean = False
        If (GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P300SLP Or GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500SLP) And objSpec.IsLTMIMSpecOn() = True And IsTDMode() = False Then blnValue = True
        Return blnValue
    End Function

    ''' <summary>
    ''' Detect if the control type used for functions designed to work for the following control types:
    ''' P300S/P500S and in TD or TL
    ''' </summary>
    ''' <returns>True if it is one of the conditions is met</returns>
    ''' <remarks>Sample codes only. It does not detect all specifications of each machine</remarks>
    Protected Function IsP300S() As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Dim blnValue As Boolean = False
        If (GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P300SLP Or GetControlType() = Okuma.CLDATAPI.Enumerations.ControlTypeEnum.P500SLP) And objSpec.IsLTMIMSpecOn() = True Then blnValue = True
        Return blnValue
    End Function

    ''' <summary>
    ''' Detect if the control type and its TD/TL mode can be used for functions designed to work for the following control types and TD/TL mode:
    ''' P300S/P500S AND IsLTMIMSpecOn() = ON
    ''' P300L/P500L AND IsLTMIMSpecOn() = ON 
    ''' </summary>
    ''' <returns>True if it is one of the conditions is met</returns>
    ''' <remarks>Sample codes only. It does not detect all specifications of each machine</remarks>
...
#Region "CTools.RegisterTools Example"
    ...
    Public Sub RegisterTools_P300S(ByRef intToolNos() As Integer)
        ...
        Dim objTurret As New Okuma.CLDATAPI.DataAPI.CTurret
        Dim intMaxPot As Integer
        Dim intMaxMultiEdgeTool As Integer = 10
        Dim intMaxMultiEdgeToolCount As Integer
        Dim sSettableToolAttributes As Okuma.CLDATAPI.Structures.SettableToolAttributes
        Dim intToolKind As Integer
        Dim intToolSize As Integer

        intMaxPot = objDATATC.GetMaxPots(Okuma.CLDATAPI.Enumerations.MagazineTypeEnum.MainMagazine)
        ...
        For Each intToolNo In intToolNos
            ...
            If IsToolInExtendedNextReadyStation(intToolNo) = False Then
                intToolEdgeNo = 0 ' register non-multi-edge tool 

                'assign at least 1 edge for multi-tool if needed
                If blnMultiEdgeTool = True And intMaxMultiEdgeToolCount < intMaxMultiEdgeTool Then
                    intToolEdgeNo = 1
                Else
                    intToolEdgeNo = 0 ' register non-multi-edge tool 
                End If

                For intPotNo = 1 To intMaxPot
                    ' assigned to different pot from previous tool
                    intPotNo = intTarget

                    If intPotNo > intMaxPot Then Exit For

                    If IsExtendedNextReadyStation(intPotNo) = False Then
                        sSettableToolAttributes = m_objATC.GetSettableToolAttributes(intPotNo)

                        objCmdATC.RegisterTool(intToolNo, intPotNo, intToolEdgeNo, GetSettableToolKindAttribute(sSettableToolAttributes, intToolKind), GetSettableToolSizeAttribute(sSettableToolAttributes, intToolSize, intPotNo), enBasicPositionSpindle, intBaseCuttingPosition(0), 0, Okuma.CLCMDAPI.Enumerations.ToolLocationEnum.Magazine)
                        'if machine can multi-edge tools then register edges to the tool
                        If blnMultiEdgeTool = True And intToolEdgeNo > 0 Then
                            RegisterEdgeNo(intToolNo)
                            intMaxMultiEdgeToolCount = intMaxMultiEdgeToolCount + 1
                        End If

                        'for next loop
                        intTarget = intTarget + 3
    ...
    Private Function GetSettableToolKindAttribute(ByVal sSettableToolAttributes As Okuma.CLDATAPI.Structures.SettableToolAttributes, ByRef intCurrent As Integer) As Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum
        Dim sSettableToolKindAttributes As Okuma.CLDATAPI.Structures.SettableToolKindAttributes
        Dim enToolKinds() As Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum
        Dim enToolKind As Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum = Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.L_Tool
        Dim blnValid As Boolean
        Dim blnSet As Boolean = False

        enToolKinds = System.Enum.GetValues(GetType(Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum))
        sSettableToolKindAttributes = sSettableToolAttributes.SettableToolKinds

        For Each enToolKind In enToolKinds
            blnValid = False

            If IsSpecCodeTurnOn(5, 3) = True Then
                If IsTookKindSupported(enToolKind, True) Then
                    blnValid = True
                End If
            Else 'non multi index spec.
                If IsTookKindSupported(enToolKind, False) Then
                    blnValid = True
                End If

            End If

            If intCurrent <> enToolKind And blnValid Then
                If enToolKind = Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.DummyTool And sSettableToolKindAttributes.DummyTool Then
                    blnSet = True
                    Exit For
                End If
                If enToolKind = Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.GrindingTool And sSettableToolKindAttributes.GrindingTool Then
                    blnSet = True
                    Exit For
                End If
                If enToolKind = Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.L_Tool And sSettableToolKindAttributes.LTool Then
                    blnSet = True
                    Exit For
                End If
                If enToolKind = Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.LMDNozzle And sSettableToolKindAttributes.LMDNozzle Then
                    blnSet = True
                    Exit For
                End If
                If enToolKind = Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.M_Tool And sSettableToolKindAttributes.MTool Then
                    blnSet = True
                    Exit For
                End If
                If enToolKind = Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.SensorTool And sSettableToolKindAttributes.SensorTool Then
                    blnSet = True
                    Exit For
                End If
            End If
        ...
        If blnSet Then
            intCurrent = enToolKind
        End If

        Return enToolKind
    End Function

    Private Function GetSettableToolSizeAttribute(ByVal sSettableToolAttributes As Okuma.CLDATAPI.Structures.SettableToolAttributes, ByRef intCurrent As Integer, intPotNo As Integer) As Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum
        Dim sSettableToolSizeAttributes As Okuma.CLDATAPI.Structures.SettableToolSizeAttributes
        Dim enToolSizes() As Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum
        Dim enToolSize As Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.Standard
        Dim blnValid As Boolean
        Dim blnSet As Boolean = False

        enToolSizes = System.Enum.GetValues(GetType(Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum))
        sSettableToolSizeAttributes = sSettableToolAttributes.SettableToolSizes

        For Each enToolSize In enToolSizes
            blnValid = False

            If enToolSize <> Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.Standard And enToolSize <> Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.E_Tool Then
                If CanInsertLargeDiameterTool(intPotNo) Then
                    blnValid = True
                End If
            End If

            If intCurrent <> enToolSize And blnValid Then
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.B_Tool And sSettableToolSizeAttributes.B_Tool Then
                    blnSet = True
                    Exit For
                End If
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.BR_Tool And sSettableToolSizeAttributes.BR_Tool Then
                    blnSet = True
                    Exit For
                End If
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.E_Tool And sSettableToolSizeAttributes.E_Tool Then
                    blnSet = True
                    Exit For
                End If
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.H_Tool And sSettableToolSizeAttributes.H_Tool Then
                    blnSet = True
                    Exit For
                End If
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.ME_Tool And sSettableToolSizeAttributes.ME_Tool Then
                    blnSet = True
                    Exit For
                End If
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.ML_Tool And sSettableToolSizeAttributes.ML_Tool Then
                    blnSet = True
                    Exit For
                End If
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.MR_Tool And sSettableToolSizeAttributes.MR_Tool Then
                    blnSet = True
                    Exit For
                End If
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.SL_Tool And sSettableToolSizeAttributes.SL_Tool Then
                    blnSet = True
                    Exit For
                End If
                If enToolSize = Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.Standard And sSettableToolSizeAttributes.Standard Then
                    blnSet = True
                    Exit For
                End If
            End If
    ...
    Private Sub ClearTool()
        ...
        If IsP300S_TD() Then
            ...
            For intPotNo = 1 To intCount
                'check for extended Next Ready stations since it is controlled by NC only
                If IsExtendedNextReadyStation(intPotNo) = False Then
            ...
            For Each intToolNo In intTools
                'check for tool in extended Next Ready stations since it is controlled by NC only
                If IsToolInExtendedNextReadyStation(intToolNo) = False Then
    ...
    ''' <summary>
    ''' Check if a pot is pre-defined as Next Ready Station by NC
    ''' </summary>
    ''' <param name="intPotNo"></param>
    ''' <returns>True if it is a Next Ready Station</returns>
    ''' <remarks>Per achine specification only</remarks>
    Public Function IsExtendedNextReadyStation(ByVal intPotNo As Integer) As Boolean
        If m_blnSupportedExtendedNextReadyStations = False Then Return False

        For Each intStation As Integer In m_intExtendedNextReadyStations
            If intStation = intPotNo Then
                Return True
                Exit Function
            End If
        Next
    ...
    ''' <summary>
    ''' Check if a tool number is currently residing in one of the Next Ready Stations
    ''' </summary>
    ''' <param name="intToolNo"></param>
    ''' <returns>True if it is residing in one of the Next Ready Stations</returns>
    ''' <remarks>Per achine specification only</remarks>
    Public Function IsToolInExtendedNextReadyStation(ByVal intToolNo As Integer) As Boolean
        Dim objATC As New Okuma.CLDATAPI.DataAPI.CATC
        If m_blnSupportedExtendedNextReadyStations = False Then Return False

        For Each intStation As Integer In m_intExtendedNextReadyStations
            If intStation = objATC.GetPotNo(intToolNo) Then
                Return True
                Exit Function
            End If
        Next
    ...
    ''' <summary>
    ''' Check if a pot can support inserting a large diameter tool
    ''' </summary>
    ''' <param name="intPotNo"></param>
    ''' <returns>True if it is.</returns>
    ''' <remarks></remarks>
    Public Function CanInsertLargeDiameterTool(ByVal intPotNo As Integer) As Boolean
        Dim objATC As New Okuma.CLDATAPI.DataAPI.CATC
        Dim intMaxPot As Integer
        Dim intAdjacentPotL As Integer
        Dim intAdjacentPotR As Integer

        intMaxPot = objATC.GetMaxPots(Okuma.CLDATAPI.Enumerations.MagazineTypeEnum.MainMagazine)
        If intPotNo = intMaxPot Then
            intAdjacentPotL = intMaxPot - 1
            intAdjacentPotR = 1
        ElseIf intPotNo = 1 Then
            intAdjacentPotL = intMaxPot
            intAdjacentPotR = 2
        Else
            intAdjacentPotL = intPotNo - 1
            intAdjacentPotR = intPotNo + 1
        End If

        If objATC.GetToolKind(intAdjacentPotL) = Okuma.CLDATAPI.Enumerations.ToolKindEnum.None And objATC.GetToolKind(intAdjacentPotR) = Okuma.CLDATAPI.Enumerations.ToolKindEnum.None Then
            Return True
        End If
    ...
    ''' </summary>
    ''' <param name="enLocation"></param>
    ''' <param name="blnMultiEdgesTool"></param>
    ''' <returns></returns>
    ''' <remarks>
    ''' Mill Tool
    ''' <para>Valid for tool assign to A Turret when CCM(NC1 No.3 bit0)=ON and MWOC(NC1 No.3 bit2)=OFF</para>
    ...
    ''' <para>Valid for tool assign to C Turret when CTSP(NC2 No.21 bit5)=ON </para>
    ''' </remarks>
    Public Function GetSettingToolKindList(ByVal enLocation As Okuma.CLCMDAPI.Enumerations.ToolLocationEnum, ByVal blnMultiEdgesTool As Boolean) As ArrayList
        Dim arSettingToolKind As New ArrayList

        If blnMultiEdgesTool = False Then
            arSettingToolKind.Add(Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.DummyTool)
        End If

        If IsSpecCodeTurnOn(3, 0) Then
            arSettingToolKind.Add(Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.L_Tool)
        End If
        ...
        If enLocation = Okuma.CLCMDAPI.Enumerations.ToolLocationEnum.A_Turret _
            ...
            If IsSpecCodeTurnOn(3, 0) And IsSpecCodeTurnOn(3, 2) = False Then
                arSettingToolKind.Add(Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.M_Tool)
            End If
        ...
        If enLocation = Okuma.CLCMDAPI.Enumerations.ToolLocationEnum.B_Turret Then
            If IsSpecCodeTurnOn(21, 5) = False Then
                arSettingToolKind.Add(Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.M_Tool)
            End If
        End If

        If enLocation = Okuma.CLCMDAPI.Enumerations.ToolLocationEnum.C_Turret Then
            If IsSpecCodeTurnOn(3, 1) And IsSpecCodeTurnOn(26, 2) = False Then
                arSettingToolKind.Add(Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum.M_Tool)
            End If
        End If
    ...
    ''' <summary>
    ''' Get list of setting tool size available for the current system
    ''' </summary>
    ''' <returns></returns>
    ''' <remarks>
    ''' </remarks>
    Public Function GetSettingToolSizeList() As ArrayList
        Dim arSettingToolSize As New ArrayList
        Dim mp As Okuma.CLDATAPI.Enumerations.MagazineEnum

        arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.H_Tool)
        arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.E_Tool)
        arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.B_Tool)
        arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.BR_Tool)
        arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.Standard)

        If CheckPLCCode3(72, 7) Then
            arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.ME_Tool)
            arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.ML_Tool)
            arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.MR_Tool)
        End If

        mp = m_objATC.GetMagazineProperty()
        If mp = Okuma.CLDATAPI.Enumerations.MagazineEnum.MatrixMagazine Then
            If CheckPLCCode(15, 4) Or CheckPLCCode(23, 2) Then
                arSettingToolSize.Add(Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum.SL_Tool)
            End If
        End If

        Return arSettingToolSize

    End Function

    Public Function CheckSpecCode(ByVal intSpecCodeIndex As Integer, ByVal intBit As Integer) As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Return objSpec.GetSpecCode(intSpecCodeIndex, intBit)
    End Function

    Public Function CheckBSpecCode(ByVal intSpecCodeIndex As Integer, ByVal intBit As Integer) As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Return objSpec.GetBSpecCode(intSpecCodeIndex, intBit)
    End Function

    Public Function CheckPLCCode(ByVal intSpecCodeIndex As Integer, ByVal intBit As Integer) As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Return objSpec.GetPLCSpecCode(intSpecCodeIndex, intBit)
    End Function

    Public Function CheckPLCCode2(ByVal intSpecCodeIndex As Integer, ByVal intBit As Integer) As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Return objSpec.GetPLCSpecCode2(intSpecCodeIndex, intBit)
    End Function

    Public Function CheckPLCCode3(ByVal intSpecCodeIndex As Integer, ByVal intBit As Integer) As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Return objSpec.GetPLCSpecCode3(intSpecCodeIndex, intBit)
    End Function

    Public Function IsSpecCodeTurnOn(ByVal intIndex As Integer, ByVal intBit As Integer) As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec
        objSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Return objSpec.GetSpecCode(intIndex, intBit)
    End Function

    Public Function IsSpecCode2TurnOn(ByVal intIndex As Integer, ByVal intBit As Integer) As Boolean
        Dim objSpec As Okuma.CLDATAPI.DataAPI.CSpec
        objSpec = New Okuma.CLDATAPI.DataAPI.CSpec
        Return objSpec.GetBSpecCode(intIndex, intBit)
    End Function

    Public Function IsTookKindSupported(ByVal enToolKind As Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum, ByVal blnMultiEdgesTool As Boolean)
        Dim ar As ArrayList
        ar = GetSettingToolKindList(Okuma.CLCMDAPI.Enumerations.ToolLocationEnum.Magazine, blnMultiEdgesTool)
        For Each enValue As Okuma.CLCMDAPI.Enumerations.SettingToolKindEnum In ar
            If enToolKind = enValue Then
                Return True
                Exit For
            End If
        Next
    ...
    Public Function IsTookSizeSupported(ByVal enToolSize As Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum)
        Dim ar As ArrayList
        ar = GetSettingToolSizeList(Okuma.CLCMDAPI.Enumerations.ToolLocationEnum.Magazine)
        For Each enValue As Okuma.CLCMDAPI.Enumerations.SettingToolSizeEnum In ar
            If enToolSize = enValue Then
                Return True
                Exit For
            End If
        Next
