...
Module modMain
    ...
    Sub Main()
        ...
        Try
            ...
            'checkl local data api version only
            ' If no local api it will use GAC libraries.
            m_strTHINCAPIPath = CurDir()

            If objFile.Exists(m_strTHINCAPIPath + "\" + "Okuma.CMDATAPI.dll") Then
                CheckFileVersion(m_strTHINCAPIPath + "\" + "Okuma.CMDATAPI.dll", 2, 7, 0, 0)
            End If

            If objFile.Exists(m_strTHINCAPIPath + "\" + "Okuma.CMCMDAPI.dll") Then
                CheckFileVersion(m_strTHINCAPIPath + "\" + "Okuma.CMCMDAPI.dll", 2, 5, 0, 0)
            End If
    ...
    Sub CheckFileVersion(ByVal strFilePath As String, ByRef intMajor As Integer, ByRef intMinor As Integer, ByRef intBuild As Integer, ByRef intRevision As Integer)
        ...
        Try
            ...
            strError = String.Format("ERROR: THINC Machining Center sample application cannot load {0} having version {1}.{2}", strFilePath, objFileVersionInfo.FileVersion(), ControlChars.CrLf + ControlChars.CrLf)
